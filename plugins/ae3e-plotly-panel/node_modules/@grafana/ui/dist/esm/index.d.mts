...
import React__default, { Component, InputHTMLAttributes, ReactElement, AriaRole, CSSProperties, ReactNode, PureComponent, ChangeEvent, ButtonHTMLAttributes, AnchorHTMLAttributes, HTMLAttributes, PropsWithChildren, RefCallback, HTMLProps, FC, ComponentType, ErrorInfo, MouseEvent as MouseEvent$1, ElementType, UIEventHandler, FormEvent, DetailedHTMLProps } from 'react';
...
import { SelectableValue, GrafanaTheme2, GrafanaTheme, Field as Field$1, IconName, FieldType, LinkTarget, LinkModel, ThemeContext, FieldReducerInfo, BootData, TimeRange, TimeZone, TimeOption, DateTime, InternalTimeZones, VariableSuggestion, NavModelItem, DataFrame, KeyValue, FieldConfigSource, InterpolateFunction, ActionModel, CSVConfig, DisplayValue, FieldSparkline, DisplayValueAlignmentFactors, DecimalCount, ThresholdsConfig, FieldColorMode, DataFrameFieldIndex, FieldConfig, DisplayProcessor, VizOrientation as VizOrientation$1, Dimensions, GraphSeriesValue, LoadingState, EventBus, DashboardCursorSync, CoreApp, AnnotationEventUIModel, DataLinkPostProcessor, GraphSeriesXY, ReduceDataOptions, ValueMapping, PanelModel, DataLink, Action, FeatureState, DataSourceJsonData, DataSourceSettings, DataSourcePluginOptionsEditorProps, SliderMarks, RegistryItem, FieldMatcherInfo, Registry, ThemeTypographyVariantTypes, ThemeSpacingTokens, ThemeShape, ThemeShadows, RelativeTimeRange, FormattedValue, PluginSignatureStatus, PluginSignatureType, DateTimeInput, Dimension, FlotDataPoint, FieldMatcher, DataHoverEvent, LegacyGraphHoverEvent, StandardEditorProps, FieldConfigEditorBuilder, PanelOptionsEditorBuilder } from '@grafana/data';
...
import { FieldValues, FieldArrayMethodProps, UseFormReturn, FieldErrors, Mode, DefaultValues, SubmitHandler, UseFieldArrayProps } from 'react-hook-form';
...
import { VizTextDisplayOptions, PercentChangeColorMode, AxisPlacement, ScaleDistribution, ScaleOrientation, ScaleDirection, StackingMode, LineConfig, BarConfig, FillConfig, PointsConfig, GraphGradientMode, GraphDrawStyle, GraphThresholdsStyleConfig, VizOrientation, GraphFieldConfig, BarGaugeDisplayMode, BarGaugeValueMode, BarGaugeNamePlacement, TooltipDisplayMode, LegendPlacement, LineStyle, LegendDisplayMode, OptionsWithTextFormatting, LineInterpolation, BarAlignment, VisibilityMode, GraphThresholdsStyleMode, VizLegendOptions, DashboardCursorSync as DashboardCursorSync$1, SortOrder, ScaleDistributionConfig, AxisConfig, HideableFieldConfig, OptionsWithLegend, OptionsWithTooltip, StackingConfig } from '@grafana/schema';
...
import { LanguageMap } from 'prismjs';
...
type Props$1i<T> = NoticeProps<SelectableValue<T>, boolean, GroupBase<SelectableValue<T>>>;
...
declare const Tooltip: React$1.ForwardRefExoticComponent<TooltipProps & React$1.RefAttributes<HTMLElement>>;
...
declare class PopoverController extends Component<Props$1h, State$6> {
    private hideTimeout;
    state: {
        show: boolean;
    };
...
interface Props$1g extends InputHTMLAttributes<HTMLInputElement> {
    label: string;
...
interface Props$1f extends Omit<InputHTMLAttributes<HTMLInputElement>, 'onReset'> {
    onReset: (event: React$1.SyntheticEvent<HTMLButtonElement>) => void;
...
declare namespace dom_d {
  export { dom_d_getNextCharacter as getNextCharacter, dom_d_getPreviousCousin as getPreviousCousin };
}
 ...
 */
declare function renderOrCallToRender<TProps = {}>(itemToRender: ((props: TProps) => React$1.ReactNode) | React$1.ReactNode, props?: TProps): React$1.ReactNode;
...
declare namespace reactUtils_d {
  export { reactUtils_d_getChildId as getChildId, reactUtils_d_renderOrCallToRender as renderOrCallToRender };
}
...
interface MenuItemProps<T = unknown> {
    ...
    onClick?: (event: React$1.MouseEvent<HTMLElement>, payload?: T) => void;
...
/** @internal */
declare const MenuItem: React$1.MemoExoticComponent<React$1.ForwardRefExoticComponent<MenuItemProps<unknown> & React$1.RefAttributes<MenuItemElement>>>;
 ...
 */
declare function logOptions(amount: number, recommendedAmount: number, id: string | undefined, ariaLabelledBy: string | undefined): void;
...
interface SelectCommonProps<T> {
    ...
    formatOptionLabel?: (item: SelectableValue<T>, formatOptionMeta: FormatOptionLabelMeta<T>) => React$1.ReactNode;
    /** Function for formatting the text that is displayed when creating a new value*/
    formatCreateLabel?: (input: string) => React$1.ReactNode;
    getOptionLabel?: (item: SelectableValue<T>) => React$1.ReactNode;
    ...
    toggleAllOptions?: {
        ...
        determineToggleAllState?: (selectedValues: Array<SelectableValue<T>>, options: Array<SelectableValue<T>>) => ToggleAllState;
    };
...
type ControlComponent<T> = React$1.ComponentType<CustomControlProps<T>>;
...
interface Props$1e extends React$1.HTMLProps<HTMLInputElement> {
    validationEvents?: ValidationEvents;
    hideErrorMessage?: boolean;
    inputRef?: React$1.LegacyRef<HTMLInputElement>;
    onBlur?: (event: React$1.FocusEvent<HTMLInputElement>, status?: LegacyInputStatus) => void;
    onFocus?: (event: React$1.FocusEvent<HTMLInputElement>, status?: LegacyInputStatus) => void;
    onChange?: (event: React$1.ChangeEvent<HTMLInputElement>, status?: LegacyInputStatus) => void;
}
interface State$5 {
    error: string | null;
}
...
declare class Input$1 extends PureComponent<Props$1e, State$5> {
    static defaultProps: {
        className: string;
    };
    state: State$5;
...
declare class Select$1<T> extends PureComponent<LegacySelectProps<T>> {
    context: React$1.ContextType<typeof ThemeContext>;
    static contextType: React$1.Context<_grafana_data.GrafanaTheme2>;
...
interface IconProps extends Omit<React$1.SVGProps<SVGElement>, 'onLoad' | 'onError' | 'ref'> {
    name: IconName;
    size?: IconSize;
    type?: IconType;
    /**
...
declare const Icon: React$1.ForwardRefExoticComponent<IconProps & React$1.RefAttributes<SVGElement>>;

type IconButtonVariant = 'primary' | 'secondary' | 'destructive';
interface BaseProps$3 extends Omit<React$1.ButtonHTMLAttributes<HTMLButtonElement>, 'aria-label'> {
    /** Name of the icon **/
    name: IconName;
...
interface BasePropsWithTooltip extends BaseProps$3 {
    /** Tooltip content to display on hover and as the aria-label */
    tooltip: PopoverContent;
    /** Position of the tooltip */
    tooltipPlacement?: TooltipPlacement;
}
interface BasePropsWithAriaLabel extends BaseProps$3 {
    /** @deprecated use aria-label instead*/
    ariaLabel?: string;
    /** Text available only for screen readers. No tooltip will be set in this case. */
    ['aria-label']: string;
}
type Props$1d = BasePropsWithTooltip | BasePropsWithAriaLabel;
declare const IconButton: React$1.ForwardRefExoticComponent<Props$1d & React$1.RefAttributes<HTMLButtonElement>>;
...
type CommonProps$1 = {
    ...
    fill?: ButtonFill;
    icon?: IconName | React$1.ReactElement;
    className?: string;
...
declare const Button: React$1.ForwardRefExoticComponent<CommonProps$1 & ButtonHTMLAttributes<HTMLButtonElement> & React$1.RefAttributes<HTMLButtonElement>>;
declare const LinkButton: React$1.ForwardRefExoticComponent<CommonProps$1 & ButtonHTMLAttributes<HTMLButtonElement> & AnchorHTMLAttributes<HTMLAnchorElement> & React$1.RefAttributes<HTMLAnchorElement>>;
declare const clearButtonStyles: (theme: GrafanaTheme2) => string;

interface Props$1c extends HTMLAttributes<HTMLDivElement> {
    className?: string;
}
declare const ButtonGroup: React$1.ForwardRefExoticComponent<Props$1c & React$1.RefAttributes<HTMLDivElement>>;
...
declare const ConfirmButton: {
    ({ children, className, closeOnConfirm, confirmText, confirmVariant, disabled, onCancel, onClick, onConfirm, size, }: Props$1b): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const DeleteButton: ({ size, disabled, onConfirm, "aria-label": ariaLabel, closeOnConfirm }: Props$1a) => react_jsx_runtime.JSX.Element;

interface Props$19 extends Omit<React$1.HTMLAttributes<HTMLDivElement>, 'content'> {
    show: boolean;
    placement?: TooltipPlacement;
    content: PopoverContent;
...
declare function Popover({ content, show, placement, className, wrapperClassName, referenceElement, renderArrow, hidePopper, ...rest }: Props$19): react_jsx_runtime.JSX.Element | undefined;
...
declare const Toggletip: React$1.MemoExoticComponent<({ children, theme, placement, content, title, closeButton, onClose, footer, fitContent, onOpen, show, }: ToggletipProps) => react_jsx_runtime.JSX.Element>;

interface Props$18 {
    className?: string;
    root?: HTMLElement;
    forwardedRef?: React$1.ForwardedRef<HTMLDivElement>;
}
declare function Portal(props: PropsWithChildren<Props$18>): React$1.ReactPortal;
/** @internal */
declare function getPortalContainer(): HTMLElement;
/** @internal */
declare function PortalContainer(): react_jsx_runtime.JSX.Element;
...
interface Props$17 {
    ...
    onScroll?: React$1.UIEventHandler;
    divId?: string;
}
/**
...
declare function TabbedContainer({ tabs, defaultTab, closeIconTooltip, onClose, testId }: TabbedContainerProps): react_jsx_runtime.JSX.Element;

interface Props$16 extends ButtonProps {
...
declare function ClipboardButton({ onClipboardCopy, onClipboardError, children, getText, icon, variant, ...buttonProps }: Props$16): react_jsx_runtime.JSX.Element;
...
declare const Cascader: React$1.FunctionComponent<{
...
declare const ButtonCascader: {
    (props: ButtonCascaderProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare function InlineToast({ referenceElement, children, suffixIcon, placement }: InlineToastProps): react_jsx_runtime.JSX.Element;
 ...
 * @public
 */
declare const LoadingPlaceholder: ({ text, className, ...rest }: LoadingPlaceholderProps) => react_jsx_runtime.JSX.Element;
...
declare function LoadingBar({ width, delay, ariaLabel }: LoadingBarProps): react_jsx_runtime.JSX.Element;
...
type ColorPickerTriggerRenderer = (props: {
    ref: React$1.RefObject<any>;
    showColorPicker: () => void;
    hideColorPicker: () => void;
}) => React$1.ReactNode;
declare const ColorPicker: React$1.FunctionComponent<{
    color: string;
    onChange: ColorPickerChangeHandler;
    children?: ColorPickerTriggerRenderer | undefined;
    enableNamedColors?: boolean | undefined;
}>;
declare const SeriesColorPicker: React$1.FunctionComponent<{
    color: string;
    onChange: ColorPickerChangeHandler;
    children?: ColorPickerTriggerRenderer | undefined;
    hidePopper?: (() => void) | undefined;
    enableNamedColors?: boolean | undefined;
...
interface Props$15 extends Omit<HTMLProps<HTMLInputElement>, 'prefix' | 'size'> {
    /** Sets the width to a multiple of 8px. Should only be used with inline forms. Setting width of the container is preferred in other cases.*/
    width?: number;
...
declare const Input: React$1.ForwardRefExoticComponent<Omit<Props$15, "ref"> & React$1.RefAttributes<HTMLInputElement>>;
...
interface ColorPickerInputProps extends Omit<Props$15, 'value' | 'onChange'> {
    value?: string;
    onChange: (color: string) => void;
    /** Format for returning the color in onChange callback, defaults to 'rgb' */
    returnColorAs?: 'rgb' | 'hex';
}
declare const ColorPickerInput: React$1.ForwardRefExoticComponent<Omit<ColorPickerInputProps, "ref"> & React$1.RefAttributes<HTMLInputElement>>;

interface SeriesColorPickerPopoverProps extends ColorPickerProps, PopoverContentProps {
    yaxis?: number;
    onToggleAxis?: () => void;
}
declare const SeriesColorPickerPopover: (props: SeriesColorPickerPopoverProps) => react_jsx_runtime.JSX.Element;
declare const SeriesColorPickerPopoverWithTheme: React$1.FunctionComponent<{
    color: string;
    onChange: ColorPickerChangeHandler;
    hidePopper?: (() => void) | undefined;
    enableNamedColors?: boolean | undefined;
...
declare const EmptySearchResult: ({ children }: Props$14) => react_jsx_runtime.JSX.Element;
...
declare const EmptyState: ({ button, children, image, message, hideImage, variant, role, }: React$1.PropsWithChildren<Props$13>) => react_jsx_runtime.JSX.Element;
...
declare class StatsPicker extends PureComponent<Props$12> {
    static defaultProps: Partial<Props$12>;
    componentDidMount(): void;
    componentDidUpdate(prevProps: Props$12): void;
    checkInput: () => void;
...
type CommonProps = {
    /** Icon name */
    icon?: IconName | React$1.ReactNode;
    /** Icon size */
    iconSize?: IconSize;
    /** Tooltip */
    tooltip?: string;
...
declare const ToolbarButton: React$1.ForwardRefExoticComponent<CommonProps & ButtonHTMLAttributes<HTMLButtonElement> & React$1.RefAttributes<HTMLButtonElement>>;

interface Props$11 extends HTMLAttributes<HTMLDivElement> {
    className?: string;
    /** Determine flex-alignment of child buttons. Needed for overflow behaviour. */
    alignment?: 'left' | 'right';
}
declare const ToolbarButtonRow: React$1.ForwardRefExoticComponent<Props$11 & React$1.RefAttributes<HTMLDivElement>>;
...
declare class RefreshPicker extends PureComponent<Props$10> {
    static offOption: {
        label: string;
        value: string;
        ariaLabel: string;
...
declare function isWeekStart(value: string): value is WeekStart;
declare global {
    interface Window {
 ...
 */
declare function getWeekStart(override?: string): WeekStart;
declare const WeekStartPicker: (props: Props$$) => react_jsx_runtime.JSX.Element;
...
declare function TimeRangePicker(props: TimeRangePickerProps): react_jsx_runtime.JSX.Element;
...
declare function TimeRangeProvider({ children }: {
    children: ReactNode;
}): react_jsx_runtime.JSX.Element;
...
declare const TimeRangeLabel: React$1.NamedExoticComponent<LabelProps$2>;

type FormInputSize = 'sm' | 'md' | 'lg' | 'auto';

interface BaseProps$2 {
...
interface AllowEmptyProps extends BaseProps$2 {
    allowEmpty: true;
    onChange: (value?: DateTime) => void;
}
interface NoAllowEmptyProps extends BaseProps$2 {
    allowEmpty?: false;
    onChange: (value: DateTime) => void;
}
type Props$_ = AllowEmptyProps | NoAllowEmptyProps;
declare const TimeOfDayPicker: ({ minuteStep, showHour, showSeconds, value, size, disabled, disabledHours, disabledMinutes, disabledSeconds, placeholder, ...restProps }: Props$_) => react_jsx_runtime.JSX.Element;
...
declare const TimeZonePicker: (props: Props$Z) => react_jsx_runtime.JSX.Element;
...
/** @public */
interface DatePickerWithInputProps extends Omit<Props$15, 'ref' | 'value' | 'onChange'> {
...
/** @public */
declare const DatePickerWithInput: ({ value, minDate, maxDate, onChange, closeOnSelect, placeholder, ...rest }: DatePickerWithInputProps) => react_jsx_runtime.JSX.Element;
...
declare const DateTimePicker: ({ date, maxDate, minDate, label, onChange, disabledHours, disabledMinutes, disabledSeconds, timeZone, showSeconds, clearable, }: Props$Y) => react_jsx_runtime.JSX.Element;
...
interface BaseProps$1<TableData extends object> {
    className?: string;
    /**
...
interface WithExpandableRow<TableData extends object> extends BaseProps$1<TableData> {
    /**
...
interface WithoutExpandableRow<TableData extends object> extends BaseProps$1<TableData> {
    renderExpandedRow?: never;
    showExpandAll?: never;
}
type Props$X<TableData extends object> = WithExpandableRow<TableData> | WithoutExpandableRow<TableData>;
/** @alpha */
declare function InteractiveTable<TableData extends object>({ className, columns, data, getRowId, headerTooltips, pageSize, renderExpandedRow, showExpandAll, fetchData, initialSortBy, }: Props$X<TableData>): react_jsx_runtime.JSX.Element;
...
declare const TagsInput: ({ placeholder, tags, onChange, width, className, disabled, addOnBlur, invalid, id, }: Props$W) => react_jsx_runtime.JSX.Element;
...
interface Props$V<T = string> extends Omit<FieldProps, 'children'> {
    ...
    children: (onChange: (newValue: T) => void) => React$1.ReactElement;
}
declare function AutoSaveField<T = string>(props: Props$V<T>): react_jsx_runtime.JSX.Element;
...
declare const Pagination: ({ currentPage, numberOfPages, onNavigate, hideWhenSinglePage, showSmallVersion, className, }: Props$U) => react_jsx_runtime.JSX.Element | null;

/**
 * @public
 */
type OnTagClick = (name: string, event: React$1.MouseEvent<HTMLElement>) => void;
...
declare const Tag: React$1.ForwardRefExoticComponent<Props$T & React$1.RefAttributes<HTMLElement>> & {
    Skeleton: (props: unknown) => react_jsx_runtime.JSX.Element;
};
...
declare const TagList: React$1.NamedExoticComponent<Omit<Props$S & React$1.RefAttributes<HTMLUListElement>, "ref"> & {
    ref?: ((instance: HTMLUListElement | null) => void | React$1.DO_NOT_USE_OR_YOU_WILL_BE_FIRED_CALLBACK_REF_RETURN_VALUES[keyof React$1.DO_NOT_USE_OR_YOU_WILL_BE_FIRED_CALLBACK_REF_RETURN_VALUES]) | React$1.RefObject<HTMLUListElement> | null | undefined;
}> & {
    readonly type: React$1.ForwardRefExoticComponent<Props$S & React$1.RefAttributes<HTMLUListElement>>;
} & {
    Skeleton: (props: unknown) => react_jsx_runtime.JSX.Element;
};
...
declare const FilterPill: ({ label, selected, onClick, icon }: FilterPillProps) => react_jsx_runtime.JSX.Element;
...
declare const ConfirmModal: ({ isOpen, title, body, description, confirmText, confirmVariant, confirmationText, dismissText, dismissVariant, alternativeText, modalClass, icon, onConfirm, onDismiss, onAlternative, confirmButtonVariant, disabled, }: ConfirmModalProps) => JSX.Element;
...
interface QueryFieldProps extends Themeable2 {
    ...
    onClick?: EventHook<React$1.MouseEvent<Element, MouseEvent>>;
...
declare const QueryField: React$1.FunctionComponent<{
    query?: string | null | undefined;
    onClick?: EventHook<React$1.MouseEvent<Element, MouseEvent>> | undefined;
...
declare const CodeEditor: React$1.FunctionComponent<{
    value: string;
    ...
    onBeforeEditorMount?: ((monaco: Monaco) => void) | undefined;
    onEditorDidMount?: ((editor: MonacoEditor, monaco: Monaco) => void) | undefined;
 ...
 * Experimental export
 **/
declare const ReactMonacoEditorLazy: (props: ReactMonacoEditorProps) => react_jsx_runtime.JSX.Element;
...
declare function Modal(props: PropsWithChildren<Props$R>): react_jsx_runtime.JSX.Element | null;
declare namespace Modal {
    var ButtonRow: typeof ModalButtonRow;
}
declare function ModalButtonRow({ leftItems, children }: {
    leftItems?: React$1.ReactNode;
    children: React$1.ReactNode;
}): react_jsx_runtime.JSX.Element;
...
/** @internal */
declare const ModalHeader: ({ icon, iconTooltip, title, children, id }: React$1.PropsWithChildren<Props$Q>) => react_jsx_runtime.JSX.Element;
...
declare const ModalTabsHeader: ({ icon, title, tabs, activeTab, onChangeTab }: Props$P) => react_jsx_runtime.JSX.Element;
...
/** @internal */
declare const ModalTabContent: ({ children }: React$1.PropsWithChildren<Props$O>) => react_jsx_runtime.JSX.Element;

interface ModalsContextState {
    component: React$1.ComponentType<any> | null;
    props: any;
    showModal: <T>(component: React$1.ComponentType<T>, props: T) => void;
    hideModal: () => void;
}
declare const ModalsContext: React$1.Context<ModalsContextState>;
...
declare const ModalRoot: () => react_jsx_runtime.JSX.Element;
declare const ModalsController: React$1.Consumer<ModalsContextState>;
...
/** @alpha */
declare const PageToolbar: React$1.MemoExoticComponent<({ title, section, parent, pageIcon, onGoBack, children, titleHref, parentHref, leftItems, isFullscreen, className, "aria-label": ariaLabel, buttonOverflowAlignment, forceShowLeftItems, }: Props$N) => react_jsx_runtime.JSX.Element>;
...
declare enum TableCellInspectorMode {
    code = "code",
    text = "text"
}
...
declare function TableCellInspector({ value, onDismiss, mode }: TableCellInspectorProps): react_jsx_runtime.JSX.Element;
...
type GetActionsFunction = (frame: DataFrame, field: Field$1, rowIndex: number, replaceVariables?: InterpolateFunction) => ActionModel[];

declare const Table: React$1.MemoExoticComponent<(props: Props$L) => react_jsx_runtime.JSX.Element>;

declare const TableInputCSV: React$1.FunctionComponent<{
    text: string;
    width: string | number;
...
declare const TabsBar: React$1.ForwardRefExoticComponent<Props$K & React$1.RefAttributes<HTMLDivElement>>;
...
interface TabProps extends HTMLProps<HTMLElement> {
    ...
    href?: string;
    icon?: IconName;
    onChangeTab?: (event: React$1.MouseEvent<HTMLElement>) => void;
    /** A number rendered next to the text. Usually used to display the number of items in a tab's view. */
    counter?: number | null;
    /** Extra content, displayed after the tab label and counter */
    suffix?: NavModelItem['tabSuffix'];
}
declare const Tab: React$1.ForwardRefExoticComponent<Omit<TabProps, "ref"> & React$1.RefAttributes<HTMLElement>>;

declare const VerticalTab: React$1.ForwardRefExoticComponent<Omit<TabProps, "ref"> & React$1.RefAttributes<HTMLAnchorElement>>;

interface Props$J extends HTMLAttributes<HTMLDivElement> {
    children: ReactNode;
}
declare const TabContent: ({ children, className, ...restProps }: Props$J) => react_jsx_runtime.JSX.Element;

interface CounterProps {
    value: number;
}
declare const Counter: ({ value }: CounterProps) => react_jsx_runtime.JSX.Element;

interface RenderUserContentAsHTMLProps<T = HTMLSpanElement> extends Omit<HTMLAttributes<T>, 'dangerouslySetInnerHTML'> {
    component?: keyof React$1.ReactHTML;
    content: string;
}
...
declare class UPlotScaleBuilder extends PlotConfigBuilder<ScaleProps, Scale> {
...
declare class Gauge extends PureComponent<Props$H> {
    ...
    draw(): void;
    renderVisualization: () => react_jsx_runtime.JSX.Element;
    render(): react_jsx_runtime.JSX.Element;
}
...
interface VizTooltipProps {
    ...
    tooltipComponent?: React$1.ComponentType<React$1.PropsWithChildren<VizTooltipContentProps>>;
...
declare const VizTooltip: {
    ({ content, position, offset }: VizTooltipProps): react_jsx_runtime.JSX.Element | null;
    displayName: string;
};
...
declare const VizTooltipContainer: {
    ({ position: { x: positionX, y: positionY }, offset: { x: offsetX, y: offsetY }, children, allowPointerEvents, className, ...otherProps }: VizTooltipContainerProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
 ...
 * @public
 */
declare const SeriesTableRow: ({ color, label, value, isActive }: SeriesTableRowProps) => react_jsx_runtime.JSX.Element;
/**
 * @public
 */
 ...
 * @public
 */
declare const SeriesTable: ({ timestamp, series }: SeriesTableProps) => react_jsx_runtime.JSX.Element;
...
type PropsWithDefaults<V, D> = Props$F<V, D> & DefaultProps;
interface State$3<V> {
    values: V[];
}
declare class VizRepeater<V, D = {}> extends PureComponent<PropsWithDefaults<V, D>, State$3<V>> {
    ...
    renderGrid(): react_jsx_runtime.JSX.Element;
    render(): react_jsx_runtime.JSX.Element;
}
 ...
 * @internal
 */
declare const LoadingIndicator: ({ onCancel, loading }: LoadingIndicatorProps) => react_jsx_runtime.JSX.Element | null;
...
interface BaseProps {
    ...
    onDragStart?: (e: React$1.PointerEvent) => void;
    selectionId?: string;
    /**
     ...
     */
    statusMessageOnClick?: (e: React$1.SyntheticEvent) => void;
    /**
...
declare const TitleItem: React$1.ForwardRefExoticComponent<TitleItemProps & React$1.RefAttributes<TitleItemElement>>;
...
type OnSelectRangeCallback = (selections: RangeSelection2D[]) => void;
interface RangeSelection1D {
    from: number;
    to: number;
}
interface RangeSelection2D {
    x?: RangeSelection1D;
    y?: RangeSelection1D;
}
...
declare const PanelContextRoot: React$1.Context<PanelContext>;
/**
 * @alpha
 */
declare const PanelContextProvider: React$1.Provider<PanelContext>;
/**
 * @alpha
 */
 ...
 * @internal
 */
interface PanelChromeType extends React$1.FC<PanelChromeProps> {
...
interface VizLayoutProps {
    width: number;
    height: number;
    legend?: React$1.ReactElement<VizLayoutLegendProps> | null;
    children: (width: number, height: number) => React$1.ReactNode;
}
/**
 * @beta
 */
...
interface VizLegendBaseProps<T> {
    ...
    onLabelClick?: (item: VizLegendItem<T>, event: React$1.MouseEvent<HTMLButtonElement>) => void;
    itemRenderer?: (item: VizLegendItem<T>, index: number) => JSX.Element;
    onLabelMouseOver?: (item: VizLegendItem, event: React$1.MouseEvent<HTMLButtonElement> | React$1.FocusEvent<HTMLButtonElement>) => void;
    onLabelMouseOut?: (item: VizLegendItem, event: React$1.MouseEvent<HTMLButtonElement> | React$1.FocusEvent<HTMLButtonElement>) => void;
    readonly?: boolean;
}
 ...
 * @public
 */
declare function VizLegend<T>({ items, thresholdItems, mappingItems, displayMode, sortBy: sortKey, seriesVisibilityChangeBehavior, sortDesc, onLabelClick, onToggleSort, placement, className, itemRenderer, readonly, isSortable, }: LegendProps<T>): react_jsx_runtime.JSX.Element | null;
...
declare const VizLegendListItem: {
    <T = unknown>({ item, onLabelClick, onLabelMouseOver, onLabelMouseOut, className, readonly, }: Props$E<T>): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
interface Props$D extends HTMLAttributes<HTMLDivElement> {
    ...
    onRemove?: (event: React$1.MouseEvent) => void;
    severity?: AlertVariant;
    children?: ReactNode;
    elevated?: boolean;
    buttonContent?: React$1.ReactNode | string;
    bottomSpacing?: number;
    topSpacing?: number;
}
declare const Alert: React$1.ForwardRefExoticComponent<Props$D & React$1.RefAttributes<HTMLDivElement>>;

/** @deprecated */
interface GraphSeriesTogglerAPI {
    onSeriesToggle: (label: string, event: React$1.MouseEvent<HTMLElement>) => void;
...
declare const ControlledCollapse: ({ isOpen, onToggle, ...otherProps }: React$1.PropsWithChildren<Props$C>) => react_jsx_runtime.JSX.Element;
declare const Collapse: {
    ({ isOpen, label, loading, collapsible, onToggle, className, children, }: React$1.PropsWithChildren<Props$C>): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const CollapsableSection: ({ label, isOpen, onToggle, className, contentClassName, children, labelId, loading, headerDataTestId, contentDataTestId, }: Props$B) => react_jsx_runtime.JSX.Element;
 ...
 * @internal
 */
declare function DataLinkButton({ link, buttonProps }: DataLinkButtonProps): react_jsx_runtime.JSX.Element;
 ...
 * @internal
 */
declare function FieldLinkList({ links }: Props$A): react_jsx_runtime.JSX.Element;

interface Props$z {
    className?: string;
}
declare const FullWidthButtonContainer: ({ className, children }: React$1.PropsWithChildren<Props$z>) => react_jsx_runtime.JSX.Element;
...
declare class ClickOutsideWrapper extends PureComponent<React$1.PropsWithChildren<Props$y>, State$2> {
    static defaultProps: {
        includeButtonPress: boolean;
        parent: (Window & typeof globalThis) | undefined;
        useCapture: boolean;
    };
    myRef: React$1.RefObject<HTMLDivElement>;
    state: {
        hasEventListener: boolean;
    };
    componentDidMount(): void;
...
declare const CallToActionCard: ({ message, callToActionElement, footer, className }: CallToActionCardProps) => react_jsx_runtime.JSX.Element;
...
interface ContextMenuProps {
    ...
    renderMenuItems?: () => React$1.ReactNode;
    /** A function that returns header element */
    renderHeader?: () => React$1.ReactNode;
}
declare const ContextMenu: React$1.MemoExoticComponent<({ x, y, onClose, focusOnOpen, renderMenuItems, renderHeader }: ContextMenuProps) => react_jsx_runtime.JSX.Element>;
...
declare const MenuGroup: {
    ({ label, ariaLabel, children }: MenuGroupProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const Menu: React$1.ForwardRefExoticComponent<MenuProps & React$1.RefAttributes<HTMLDivElement>> & {
    Item: React$1.MemoExoticComponent<React$1.ForwardRefExoticComponent<MenuItemProps<unknown> & React$1.RefAttributes<MenuItemElement>>>;
...
interface WithContextMenuProps {
    ...
    children: (props: {
        openMenu: React$1.MouseEventHandler<HTMLElement>;
    }) => JSX.Element;
...
declare const WithContextMenu: ({ children, renderMenuItems, focusOnOpen }: WithContextMenuProps) => react_jsx_runtime.JSX.Element;

interface DataLinksInlineEditorBaseProps<T extends DataLink | Action> {
    type: 'link' | 'action';
    items?: T[];
    onChange: (items: T[]) => void;
    data: DataFrame[];
    children: (item: T, index: number, onSave: (index: number, item: T) => void, onCancel: (index: number) => void) => React.ReactNode;
}
/** @internal */
declare function DataLinksInlineEditorBase<T extends DataLink | Action>({ type, items, onChange, data, children, }: DataLinksInlineEditorBaseProps<T>): react_jsx_runtime.JSX.Element;
...
declare const DataLinksInlineEditor: ({ links, getSuggestions, showOneClick, ...rest }: DataLinksInlineEditorProps) => react_jsx_runtime.JSX.Element;
...
declare const DataLinkInput: React$1.MemoExoticComponent<({ value, onChange, suggestions, placeholder, }: DataLinkInputProps) => react_jsx_runtime.JSX.Element>;
...
interface DataLinksContextMenuApi {
    openMenu?: React$1.MouseEventHandler<HTMLOrSVGElement>;
    targetClassName?: string;
}
declare const DataLinksContextMenu: ({ children, links, style }: DataLinksContextMenuProps) => react_jsx_runtime.JSX.Element;

interface Props$x extends React$1.HTMLAttributes<HTMLDivElement> {
    color?: string;
    gradient?: string;
    lineStyle?: LineStyle;
}
declare const SeriesIcon: React$1.MemoExoticComponent<React$1.ForwardRefExoticComponent<Props$x & React$1.RefAttributes<HTMLDivElement>>>;

interface InfoBoxProps extends Omit<React$1.HTMLAttributes<HTMLDivElement>, 'title'> {
    children: React$1.ReactNode;
    /** Title of the box */
    title?: string | JSX.Element;
...
declare const InfoBox: React$1.MemoExoticComponent<React$1.ForwardRefExoticComponent<InfoBoxProps & React$1.RefAttributes<HTMLDivElement>>>;
...
/** @deprecated use Alert with severity info */
declare const FeatureInfoBox: React$1.MemoExoticComponent<React$1.ForwardRefExoticComponent<FeatureInfoBoxProps & React$1.RefAttributes<HTMLDivElement>>>;

interface FeatureBadgeProps {
    featureState: FeatureState;
    tooltip?: string;
}
declare const FeatureBadge: ({ featureState, tooltip }: FeatureBadgeProps) => react_jsx_runtime.JSX.Element;
...
declare class JSONFormatter extends PureComponent<Props$w> {
    private wrapperRef;
    static defaultProps: {
        open: number;
        config: {
...
declare class ErrorBoundary extends PureComponent<Props$v, State$1> {
    readonly state: State$1;
    componentDidCatch(error: Error, errorInfo: ErrorInfo): void;
    componentDidUpdate(prevProps: Props$v): void;
    render(): ReactNode;
}
/**
...
interface Props$u extends ErrorBoundaryApi {
    title: string;
}
declare const ErrorWithStack: {
    ({ error, errorInfo, title }: Props$u): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
interface AzureAuthSettings {
    ...
    readonly azureSettingsUI?: React$1.ComponentType<HttpSettingsBaseProps>;
}
...
interface HttpSettingsProps extends HttpSettingsBaseProps {
    ...
    renderSigV4Editor?: React$1.ReactNode;
    /** Show the Secure Socks Datasource Proxy toggle option */
    secureSocksDSProxyEnabled?: boolean;
}

declare const DataSourceHttpSettings: (props: HttpSettingsProps) => react_jsx_runtime.JSX.Element;
...
declare class CustomHeadersSettings extends PureComponent<Props$t, State> {
    state: State;
...
interface Props$s<T extends DataSourceJsonData> extends Pick<DataSourcePluginOptionsEditorProps<T>, 'options' | 'onOptionsChange'> {
}
interface AlertingConfig extends DataSourceJsonData {
    manageAlerts?: boolean;
}
declare function AlertingSettings<T extends AlertingConfig>({ options, onOptionsChange }: Props$s<T>): JSX.Element;
...
declare function SecureSocksProxySettings<T extends SecureSocksProxyConfig>({ options, onOptionsChange, }: Props$r<T>): JSX.Element;

declare const TLSAuthSettings: ({ dataSourceConfig, onChange }: HttpSettingsBaseProps) => react_jsx_runtime.JSX.Element;
...
declare const CertificationKey: ({ hasCert, label, onChange, onClick, placeholder }: Props$q) => react_jsx_runtime.JSX.Element;
 ...
 */
interface PropsWithDeprecatedSize extends Omit<Props$p, 'size'> {
    size?: number | string;
}
/**
 * @public
 */
declare const Spinner: ({ className, inline, iconClassName, style, size, }: Props$p | PropsWithDeprecatedSize) => react_jsx_runtime.JSX.Element;
...
declare function FadeTransition(props: Props$o): react_jsx_runtime.JSX.Element;
...
declare function SlideOutTransition(props: Props$n): react_jsx_runtime.JSX.Element;
...
declare function Segment<T>({ options, value, onChange, Component, className, allowCustomValue, allowEmptyValue, placeholder, disabled, inputMinWidth, inputPlaceholder, onExpandedChange, autofocus, ...rest }: React$1.PropsWithChildren<SegmentSyncProps<T>>): react_jsx_runtime.JSX.Element;
 ...
 */
interface Props$m<T> extends Omit<HTMLProps<HTMLDivElement>, 'value' | 'onChange'> {
    value?: T | SelectableValue<T>;
    options: Array<SelectableValue<T>>;
    onChange: (item: SelectableValue<T>) => void;
    /**
...
/** @internal */
declare function SegmentSelect<T>({ value, placeholder, options, onChange, onClickOutside, loadOptions, width: widthPixels, noOptionsMessage, allowCustomValue, allowEmptyValue, ...rest }: React$1.PropsWithChildren<Props$m<T>>): react_jsx_runtime.JSX.Element;
...
declare function SegmentInput({ value: initialValue, onChange, Component, className, placeholder, inputPlaceholder, disabled, autofocus, onExpandedChange, ...rest }: React$1.PropsWithChildren<SegmentInputProps>): react_jsx_runtime.JSX.Element;
...
declare function Drawer({ children, onClose, closeOnMaskClick, scrollableContent, title, subtitle, width, size, tabs, }: Props$l): react_jsx_runtime.JSX.Element;
...
declare const Slider: {
    ({ min, max, onChange, onAfterChange, orientation, reverse, step, value, ariaLabelForHandle, marks, included, }: SliderProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const RangeSlider: {
    ({ min, max, onChange, onAfterChange, orientation, reverse, step, formatTooltipResult, value, tooltipAlwaysVisible, }: RangeSliderProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
interface FormProps<T extends FieldValues> extends Omit<HTMLProps<HTMLFormElement>, 'onSubmit' | 'children'> {
    ...
    children: (api: FormAPI<T>) => React$1.ReactNode;
 ...
 */
declare function Form<T extends FieldValues>({ defaultValues, onSubmit, validateOnMount, validateFieldsOnMount, children, validateOn, maxWidth, ...htmlProps }: FormProps<T>): react_jsx_runtime.JSX.Element;
...
declare function ValuePicker<T>({ 'aria-label': ariaLabel, label, icon, options, onChange, variant, minWidth, size, isFullWidth, menuPlacement, fill, buttonCss, }: ValuePickerProps<T>): react_jsx_runtime.JSX.Element;

interface FieldMatcherUIRegistryItem<TOptions> extends RegistryItem {
    component: React$1.ComponentType<MatcherUIProps<TOptions>>;
...
interface Props$k extends AnchorHTMLAttributes<HTMLAnchorElement> {
}
/**
 * @alpha
 */
declare const Link: React$1.ForwardRefExoticComponent<Props$k & React$1.RefAttributes<HTMLAnchorElement>>;
...
declare const TextLink: React$1.ForwardRefExoticComponent<TextLinkProps & React$1.RefAttributes<HTMLAnchorElement>>;

interface TextProps extends Omit<React$1.HTMLAttributes<HTMLElement>, 'className' | 'style'> {
    ...
    children: NonNullable<React$1.ReactNode>;
}
...
type Wrap = boolean | 'nowrap' | 'wrap' | 'wrap-reverse';
...
interface BoxProps extends FlexProps, SizeProps, Omit<React$1.HTMLAttributes<HTMLElement>, 'className' | 'style'> {
...
declare const Box: React$1.ForwardRefExoticComponent<BoxProps & {
    children?: React$1.ReactNode | undefined;
} & React$1.RefAttributes<HTMLElement>>;

interface StackProps extends FlexProps, SizeProps, Omit<React$1.HTMLAttributes<HTMLElement>, 'className' | 'style'> {
    gap?: ResponsiveProp<ThemeSpacingTokens>;
    alignItems?: ResponsiveProp<AlignItems>;
    justifyContent?: ResponsiveProp<JustifyContent>;
    direction?: ResponsiveProp<Direction>;
...
declare const Space: ({ v, h, layout }: SpaceProps) => react_jsx_runtime.JSX.Element;

interface Props$j extends Omit<BoxProps, 'display' | 'direction' | 'element' | 'flex' | 'position'> {
    showScrollIndicators?: boolean;
...
declare const ScrollContainer: React$1.ForwardRefExoticComponent<Props$j & {
    children?: React$1.ReactNode | undefined;
} & React$1.RefAttributes<HTMLDivElement>>;

interface LabelProps$1 extends React$1.LabelHTMLAttributes<HTMLLabelElement> {
    children: React$1.ReactNode;
    description?: React$1.ReactNode;
    category?: React$1.ReactNode[];
}
declare const Label$1: ({ children, description, className, category, ...labelProps }: LabelProps$1) => react_jsx_runtime.JSX.Element;

interface LabelProps extends React$1.HTMLAttributes<HTMLLegendElement> {
    children: string | ReactNode;
    description?: string;
}
declare const Legend: ({ children, className, ...legendProps }: LabelProps) => react_jsx_runtime.JSX.Element;
...
declare const FieldSet: ({ label, children, className, ...rest }: Props$i) => react_jsx_runtime.JSX.Element;
...
declare const FieldValidationMessage: ({ children, horizontal, className, }: React$1.PropsWithChildren<FieldValidationMessageProps>) => react_jsx_runtime.JSX.Element;
...
declare const InlineField: {
    ({ children, label, tooltip, labelWidth, invalid, loading, disabled, required, className, htmlFor, grow, shrink, error, transparent, interactive, validationMessageHorizontalOverflow, ...htmlProps }: Props$h): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const InlineSegmentGroup: {
    ({ children, className, grow, ...htmlProps }: React$1.PropsWithChildren<Props$g>): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const InlineLabel: ({ children, className, tooltip, width, transparent, interactive, as: Component, ...rest }: Props$f) => react_jsx_runtime.JSX.Element;

interface Props$e extends Omit<HTMLProps<HTMLDivElement>, 'css'> {
    children: ReactNode | ReactNode[];
}
declare const InlineFieldRow: ({ children, className, ...htmlProps }: Props$e) => react_jsx_runtime.JSX.Element;
...
declare function resetSelectStyles(theme: GrafanaTheme2): {
    ...
    input: (originalStyles: CSSObjectWithLabel) => {
        ...
        zoom?: csstype.Property.Zoom | readonly NonNullable<csstype.Property.Zoom | undefined>[] | readonly ("normal" | "reset" | (string & {}) | csstype.Globals)[] | undefined;
        all?: csstype.Globals | readonly NonNullable<csstype.Globals | undefined>[] | readonly csstype.Globals[] | undefined;
        animation?: csstype.Property.Animation<string & {}> | readonly NonNullable<csstype.Property.Animation<string & {}> | undefined>[] | readonly ("linear" | "auto" | "none" | "normal" | "reverse" | "paused" | (string & {}) | csstype.Globals | "ease" | "ease-in" | "ease-in-out" | "ease-out" | "step-end" | "step-start" | "alternate" | "alternate-reverse" | "backwards" | "both" | "forwards" | "infinite" | "running")[] | undefined;
        ...
        borderImage?: csstype.Property.BorderImage | readonly NonNullable<csstype.Property.BorderImage | undefined>[] | readonly ("none" | "repeat" | "round" | (string & {}) | csstype.Globals | "stretch" | "space")[] | undefined;
        ...
        maskBorder?: csstype.Property.MaskBorder | readonly NonNullable<csstype.Property.MaskBorder | undefined>[] | readonly ("none" | "repeat" | "alpha" | "round" | (string & {}) | csstype.Globals | "stretch" | "space" | "luminance")[] | undefined;
        ...
        MozBorderImage?: csstype.Property.BorderImage | readonly NonNullable<csstype.Property.BorderImage | undefined>[] | readonly ("none" | "repeat" | "round" | (string & {}) | csstype.Globals | "stretch" | "space")[] | undefined;
        ...
        WebkitBorderImage?: csstype.Property.BorderImage | readonly NonNullable<csstype.Property.BorderImage | undefined>[] | readonly ("none" | "repeat" | "round" | (string & {}) | csstype.Globals | "stretch" | "space")[] | undefined;
        ...
        WebkitMaskBoxImage?: csstype.Property.MaskBorder | readonly NonNullable<csstype.Property.MaskBorder | undefined>[] | readonly ("none" | "repeat" | "alpha" | "round" | (string & {}) | csstype.Globals | "stretch" | "space" | "luminance")[] | undefined;
        ...
        OBorderImage?: csstype.Property.BorderImage | readonly NonNullable<csstype.Property.BorderImage | undefined>[] | readonly ("none" | "repeat" | "round" | (string & {}) | csstype.Globals | "stretch" | "space")[] | undefined;
...
declare const SelectContainer: <Option, isMulti extends boolean, Group extends GroupBase<Option>>(props: SelectContainerProps<Option, isMulti, Group>) => react_jsx_runtime.JSX.Element;

/** @deprecated Use Combobox component instead */
declare function Select<T, Rest = {}>(props: SelectCommonProps<T> & Rest): react_jsx_runtime.JSX.Element;
declare function MultiSelect<T, Rest = {}>(props: MultiSelectCommonProps<T> & Rest): react_jsx_runtime.JSX.Element;
interface AsyncSelectProps<T> extends Omit<SelectCommonProps<T>, 'options'>, SelectAsyncProps<T> {
    value?: T | SelectableValue<T> | null;
}
/** @deprecated Use Combobox component instead */
declare function AsyncSelect<T, Rest = {}>(props: AsyncSelectProps<T> & Rest): react_jsx_runtime.JSX.Element;
/** @deprecated Use Combobox component instead - it's virtualised by default! */
declare function VirtualizedSelect<T, Rest = {}>(props: VirtualizedSelectProps<T> & Rest): react_jsx_runtime.JSX.Element;
/** @deprecated Use Combobox component instead - it's virtualised by default! */
declare function AsyncVirtualizedSelect<T, Rest = {}>(props: VirtualizedSelectAsyncProps<T> & Rest): react_jsx_runtime.JSX.Element;
...
declare function AsyncMultiSelect<T, Rest = {}>(props: AsyncMultiSelectProps<T> & Rest): react_jsx_runtime.JSX.Element;
...
declare const SelectMenuOptions: {
    ({ children, data, innerProps, innerRef, isFocused, isSelected, renderOptionLabel, }: React$1.PropsWithChildren<SelectMenuOptionProps<unknown>>): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
type ComboboxOption<T extends string | number = string> = {
    label?: string;
    value: T;
    description?: string;
    group?: string;
...
interface ComboboxBaseProps<T extends string | number> extends Pick<Props$15, 'placeholder' | 'autoFocus' | 'id' | 'aria-labelledby' | 'disabled' | 'loading' | 'invalid'> {
    /**
 ...
 * @alpha
 */
declare const Combobox: <T extends string | number>(props: ComboboxProps<T>) => react_jsx_runtime.JSX.Element;
...
declare const MultiCombobox: <T extends string | number>(props: MultiComboboxProps<T>) => react_jsx_runtime.JSX.Element;
 ...
 */
declare const HorizontalGroup: ({ children, spacing, justify, align, wrap, width, height, }: Omit<LayoutProps, "orientation">) => react_jsx_runtime.JSX.Element;
/**
 * @deprecated use Stack component with the "column" direction instead
 */
declare const VerticalGroup: ({ children, spacing, justify, align, width, height, }: Omit<LayoutProps, "orientation" | "wrap">) => react_jsx_runtime.JSX.Element;
declare const Container: ({ children, padding, margin, grow, shrink }: React$1.PropsWithChildren<ContainerProps>) => react_jsx_runtime.JSX.Element;

type BadgeColor = 'blue' | 'red' | 'green' | 'orange' | 'purple' | 'darkgrey';
...
declare function RadioButtonGroup<T>({ options, value, onChange, onClick, disabled, disabledOptions, size, id, className, fullWidth, autoFocus, 'aria-label': ariaLabel, invalid, }: RadioButtonGroupProps<T>): react_jsx_runtime.JSX.Element;
declare namespace RadioButtonGroup {
    var displayName: string;
}

interface RadioButtonDotProps<T> extends Omit<React$1.HTMLProps<HTMLInputElement>, 'label' | 'value' | 'onChange' | 'type'> {
    id: string;
    name: string;
    checked?: boolean;
    value?: T;
...
declare const RadioButtonDot: <T extends string | number | readonly string[]>({ id, name, label, checked, value, disabled, description, onChange, ...props }: RadioButtonDotProps<T>) => react_jsx_runtime.JSX.Element;
...
declare function RadioButtonList<T extends string | number | readonly string[]>({ name, id, options, value, onChange, className, disabled, disabledOptions, }: RadioButtonListProps<T>): react_jsx_runtime.JSX.Element;

interface Props$d extends Props$15 {
     ...
     */
    onCommitChange?: (event: React$1.FormEvent<HTMLInputElement>) => void;
    /** @deprecated Use `value` and `onChange` instead to manage the value in the parent as a controlled input */
    defaultValue?: string | number | readonly string[];
}
declare const AutoSizeInput: React$1.ForwardRefExoticComponent<Omit<Props$d, "ref"> & React$1.RefAttributes<HTMLInputElement>>;
...
declare const FilterInput: React$1.ForwardRefExoticComponent<Omit<Props$c, "ref"> & React$1.RefAttributes<HTMLInputElement>>;
...
declare const SecretInput: ({ isConfigured, onReset, ...props }: Props$b) => react_jsx_runtime.JSX.Element;

interface Props$a extends Omit<HTMLProps<HTMLTextAreaElement>, 'size'> {
    /** Show an invalid state around the input */
    invalid?: boolean;
}
declare const TextArea: React$1.ForwardRefExoticComponent<Omit<Props$a, "ref"> & React$1.RefAttributes<HTMLTextAreaElement>>;
 ...
 */
declare const SecretTextArea: ({ isConfigured, onReset, ...props }: Props$9) => react_jsx_runtime.JSX.Element;
...
declare const Switch: React$1.ForwardRefExoticComponent<Omit<Props$8, "ref"> & React$1.RefAttributes<HTMLInputElement>>;
...
declare const InlineSwitch: React$1.ForwardRefExoticComponent<Omit<InlineSwitchProps, "ref"> & React$1.RefAttributes<HTMLInputElement>>;
...
interface CheckboxProps extends Omit<HTMLProps<HTMLInputElement>, 'value'> {
    ...
    description?: string | React$1.ReactElement;
...
declare const FileUpload: ({ onFileUpload, className, children, accept, size, showFileName, }: React$1.PropsWithChildren<Props$7>) => react_jsx_runtime.JSX.Element;
...
declare function FileDropzone({ options, children, readAs, onLoad, fileListRenderer, onFileRemove }: FileDropzoneProps): react_jsx_runtime.JSX.Element;
...
declare function FileListItem({ file: customFile, removeFile }: FileListItemProps): react_jsx_runtime.JSX.Element;
...
declare const TimeRangeInput: ({ value, onChange, onChangeTimeZone, clearable, weekStart, hideTimeZone, timeZone, placeholder, isReversed, hideQuickRanges, disabled, showIcon, }: TimeRangeInputProps) => react_jsx_runtime.JSX.Element;
 ...
 * @internal
 */
declare function RelativeTimeRangePicker(props: RelativeTimeRangePickerProps): react_jsx_runtime.JSX.Element;
...
declare const CardContainer: ({ children, disableEvents, disableHover, isSelected, className, href, ...props }: CardContainerProps) => react_jsx_runtime.JSX.Element;
...
declare const Tags: {
    ({ children, className }: ChildProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const Meta: React$1.MemoExoticComponent<({ children, className, separator }: ChildProps & {
    separator?: string;
}) => react_jsx_runtime.JSX.Element | null>;
declare const Actions: {
    ({ children, disabled, className }: ChildProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const FormattedValueDisplay: {
    ({ value, className, style, ...htmlProps }: Props$5): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const ButtonSelectComponent: {
    <T>(props: Props$4<T>): react_jsx_runtime.JSX.Element;
    displayName: string;
};
declare const ButtonSelect: typeof ButtonSelectComponent;
...
declare const Dropdown: React$1.MemoExoticComponent<({ children, overlay, placement, offset, onVisibleChange }: Props$3) => react_jsx_runtime.JSX.Element>;
...
declare const PluginSignatureBadge: {
    ({ status, color, signatureType, signatureOrg, ...otherProps }: PluginSignatureBadgeProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare const UserIcon: ({ userView, className, children, onClick, showTooltip, }: PropsWithChildren<UserIconProps>) => react_jsx_runtime.JSX.Element;
...
declare const UsersIndicator: ({ users, onClick, limit }: UsersIndicatorProps) => react_jsx_runtime.JSX.Element | null;
...
declare const Avatar: ({ src, alt, width, height }: AvatarProps) => react_jsx_runtime.JSX.Element;
...
declare const InlineFormLabel: ({ children, isFocused, isInvalid, className, htmlFor, tooltip, width, interactive, ...rest }: Props$2) => react_jsx_runtime.JSX.Element;
...
declare const Divider: {
    ({ direction, spacing }: DividerProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
declare function useSplitter(options: UseSplitterOptions): {
    ...
    splitterProps: {
        onPointerUp: (e: React$1.PointerEvent<HTMLDivElement>) => void;
        onPointerDown: (e: React$1.PointerEvent<HTMLDivElement>) => void;
        onPointerMove: (e: React$1.PointerEvent<HTMLDivElement>) => void;
        onKeyDown: (e: React$1.KeyboardEvent<HTMLDivElement>) => void;
        onKeyUp: (e: React$1.KeyboardEvent<HTMLDivElement>) => void;
        onDoubleClick: () => void;
        onBlur: () => void;
        ref: React$1.MutableRefObject<HTMLDivElement | null>;
        style: {
...
declare class UPlotChart extends Component<PlotProps, UPlotChartState> {
    plotContainer: React$1.RefObject<HTMLDivElement>;
    plotCanvasBBox: React$1.RefObject<DOMRect>;
...
declare const PlotLegend: React$1.MemoExoticComponent<({ data, config, placement, calcs, displayMode, ...vizLayoutLegendProps }: PlotLegendProps) => react_jsx_runtime.JSX.Element>;

interface EventsCanvasProps {
    id: string;
    config: UPlotConfigBuilder;
    events: DataFrame[];
    renderEventMarker: (dataFrame: DataFrame, dataFrameFieldIndex: DataFrameFieldIndex) => React$1.ReactNode;
    mapEventToXYCoords: (dataFrame: DataFrame, dataFrameFieldIndex: DataFrameFieldIndex) => {
        x: number;
        y: number;
    } | undefined;
}
declare function EventsCanvas({ id, events, renderEventMarker, mapEventToXYCoords, config }: EventsCanvasProps): react_jsx_runtime.JSX.Element | null;
...
declare const Marker: ({ x, y, children }: React$1.PropsWithChildren<MarkerProps>) => react_jsx_runtime.JSX.Element;
 ...
 */
declare const XYCanvas: ({ children, left, top }: React$1.PropsWithChildren<XYCanvasProps>) => react_jsx_runtime.JSX.Element;
...
interface TooltipPlugin2Props {
    ...
    syncMode?: DashboardCursorSync$1;
    syncScope?: string;
    queryZoom?: (range: {
        from: number;
        to: number;
    ...
    render: (u: uPlot, dataIdxs: Array<number | null>, seriesIdx: number | null, isPinned: boolean, dismiss: () => void, timeRange: TimeRange2 | null, viaSync: boolean, dataLinks: LinkModel[]) => React$1.ReactNode;
    maxWidth?: number;
}
 ...
 * @alpha
 */
declare const TooltipPlugin2: ({ config, hoverMode, render, clientZoom, queryZoom, onSelectRange, maxWidth, syncMode, syncScope, getDataLinks, }: TooltipPlugin2Props) => React$1.ReactPortal | null;
 ...
 * @alpha
 */
declare const EventBusPlugin: ({ config, eventBus, frame }: EventBusPluginProps) => null;
...
type OnLabelClick = (name: string, value: string | undefined, event: React$1.MouseEvent<HTMLElement>) => void;
 ...
 * @internal
 */
declare const Label: React$1.ForwardRefExoticComponent<Props$1 & React$1.RefAttributes<HTMLButtonElement>>;

type Props = DetailedHTMLProps<HTMLAttributes<HTMLDivElement>, HTMLDivElement>;
declare const PanelContainer: ({ children, className, ...props }: Props) => react_jsx_runtime.JSX.Element;
...
declare class Graph extends PureComponent<GraphProps, GraphState> {
    ...
    renderTooltip: () => React$1.ReactElement<VizTooltipProps, string | React$1.JSXElementConstructor<any>> | null;
    renderContextMenu: () => react_jsx_runtime.JSX.Element | null;
    getBarWidth: () => number;
    draw(): void;
    render(): react_jsx_runtime.JSX.Element;
}
...
declare const GraphWithLegend: (props: GraphWithLegendProps) => react_jsx_runtime.JSX.Element;
...
/** @internal */
declare const GraphContextMenu: ({ getContextMenuSource, timeZone, itemsGroup, dimensions, contextDimensions, ...otherProps }: GraphContextMenuProps) => react_jsx_runtime.JSX.Element;
/** @internal */
...
interface GraphNGProps extends Themeable2 {
    ...
    children?: (builder: UPlotConfigBuilder, alignedFrame: DataFrame) => React$1.ReactNode;
    prepConfig: (alignedFrame: DataFrame, allFrames: DataFrame[], getTimeRange: () => TimeRange) => UPlotConfigBuilder;
    propsToDiff?: Array<string | PropDiffFn>;
    preparePlotFrame?: (frames: DataFrame[], dimFields: XYFieldMatchers) => DataFrame | null;
    renderLegend: (config: UPlotConfigBuilder) => React$1.ReactElement | null;
    /**
...
declare class GraphNG extends Component<GraphNGProps, GraphNGState> {
    static contextType: React$1.Context<PanelContext>;
...
declare const TimeSeries: React$1.FunctionComponent<{
    fields?: XYFieldMatchers | undefined;
    width: number;
    options?: Record<string, any> | undefined;
    timeZone: _grafana_data.TimeZone[] | _grafana_data.TimeZone;
    legend: schema.VizLegendOptions;
    height: number;
    structureRev?: number | undefined;
    children?: ((builder: UPlotConfigBuilder, alignedFrame: DataFrame) => React$1.ReactNode) | undefined;
    timeRange: TimeRange;
    frames: DataFrame[];
    renderers?: Renderers | undefined;
    tweakScale?: ((opts: ScaleProps, forField: _grafana_data.Field) => ScaleProps) | undefined;
    tweakAxis?: ((opts: AxisProps, forField: _grafana_data.Field) => AxisProps) | undefined;
...
declare const useGraphNGContext: () => {
    ...
    getXAxisField: () => Field$1<any> | null;
...
interface TooltipPluginProps {
    ...
    sortOrder?: SortOrder;
    sync?: () => DashboardCursorSync;
    renderTooltip?: (alignedFrame: DataFrame, seriesIdx: number | null, datapointIdx: number | null) => React$1.ReactNode;
}
/**
 * @alpha
 */
declare const TooltipPlugin: ({ mode, sortOrder, sync, timeZone, config, renderTooltip, ...otherProps }: TooltipPluginProps) => react_jsx_runtime.JSX.Element | null;

/** @alpha */
interface ElementSelectionContextState {
    /**
...
interface ElementSelectionContextItem {
    id: string;
}
declare const ElementSelectionContext: React__default.Context<ElementSelectionContextState | undefined>;
interface UseElementSelectionResult {
    isSelected?: boolean;
    isSelectable?: boolean;
    onSelect?: (evt: React__default.PointerEvent) => void;
}
declare function useElementSelection(id: string | undefined): UseElementSelectionResult;

/** @deprecated Please use non-legacy versions of these components */
declare const LegacyForms: {
    SecretFormField: {
        ({ label, labelWidth, inputWidth, onReset, isConfigured, tooltip, placeholder, interactive, ...inputProps }: Props$1f): react_jsx_runtime.JSX.Element;
        displayName: string;
    };
    FormField: {
        ({ label, tooltip, labelWidth, inputWidth, inputEl, className, interactive, ...inputProps }: Props$1g): react_jsx_runtime.JSX.Element;
        displayName: string;
    };
    Select: typeof Select$1;
    AsyncSelect: typeof AsyncSelect$1;
    IndicatorsContainer: <T>(props: react_select.IndicatorsContainerProps<_grafana_data.SelectableValue<T>>) => react_jsx_runtime.JSX.Element;
    NoOptionsMessage: <T extends unknown>(props: Props$1i<T>) => react_jsx_runtime.JSX.Element;
    Input: typeof Input$1;
    Switch: React$1.FunctionComponent<{
        label: string;
...
/** @public */
declare const withTheme: <P extends Themeable, S extends {} = {}>(Component: React$1.ComponentType<P>) => React$1.FunctionComponent<Subtract<P, Themeable>> & S;
/** @alpha */
declare const withTheme2: <P extends Themeable2, S extends {} = {}>(Component: React$1.ComponentType<P>) => React$1.FunctionComponent<Subtract<P, Themeable2>> & S;
...
declare namespace mixins_d {
  export { mixins_d_cardChrome as cardChrome, mixins_d_focusCss as focusCss, mixins_d_getFocusStyles as getFocusStyles, mixins_d_getMouseFocusStyles as getMouseFocusStyles, mixins_d_getTooltipContainerStyles as getTooltipContainerStyles, mixins_d_hoverColor as hoverColor, mixins_d_listItem as listItem, mixins_d_listItemSelected as listItemSelected, mixins_d_mediaUp as mediaUp };
}

/**
 * @alpha
 */
declare function addAxisConfig(builder: FieldConfigEditorBuilder<AxisConfig>, defaultConfig: AxisConfig): void;
/**
 * @internal
 */
declare const ScaleDistributionEditor: ({ value, onChange }: StandardEditorProps<ScaleDistributionConfig>) => react_jsx_runtime.JSX.Element;
/** @internal */
declare function addAxisWidth(builder: FieldConfigEditorBuilder<AxisConfig>): void;
/** @internal */
declare function addAxisPlacement(builder: FieldConfigEditorBuilder<AxisConfig>, optionsFilter?: (placement: AxisPlacement) => boolean): void;
 ...
 * @alpha
 */
declare function addLegendOptions<T extends OptionsWithLegend>(builder: PanelOptionsEditorBuilder<T>, includeLegendCalcs?: boolean, showLegend?: boolean): void;
...
declare const index_d_addAxisPlacement: typeof addAxisPlacement;
declare const index_d_addAxisWidth: typeof addAxisWidth;
...
declare namespace index_d {
  export { index_d_ScaleDistributionEditor as ScaleDistributionEditor, index_d_StackingEditor as StackingEditor, index_d_addAxisConfig as addAxisConfig, index_d_addAxisPlacement as addAxisPlacement, index_d_addAxisWidth as addAxisWidth, index_d_addHideFrom as addHideFrom, index_d_addLegendOptions as addLegendOptions, index_d_addStackingConfig as addStackingConfig, index_d_addTextSizeOptions as addTextSizeOptions, index_d_addTooltipOptions as addTooltipOptions };
}
...
declare function RunnerPlugin({ handler }: {
    handler?: (e: React$1.KeyboardEvent) => void;
}): Plugin;
 ...
 */
declare function SlatePrism(optsParam?: OptionsFormat, prismLanguages?: LanguageMap): Plugin;
...
export { ALERTING_COLOR, type ActionMeta, type AdHocFilterItem, Alert, type AlertVariant, AlertingSettings, AsyncMultiSelect, AsyncSelect, type AsyncSelectProps, AsyncVirtualizedSelect, AutoSaveField, AutoSizeInput, Avatar, Badge, type BadgeColor, type BadgeProps, BarGauge, BigValue, BigValueColorMode, BigValueGraphMode, BigValueJustifyMode, BigValueTextMode, Box, BracesPlugin, Label as BrowserLabel, Button, ButtonCascader, ButtonGroup, type ButtonProps, ButtonSelect, type ButtonVariant, CallToActionCard, Card, CardContainer, type CardContainerProps, type Props$6 as CardProps, Cascader, type CascaderOption, CertificationKey, Checkbox, ClearPlugin, ClickOutsideWrapper, ClipboardButton, ClipboardPlugin, CodeEditor, type MonacoOptions as CodeEditorMonacoOptions, type CodeEditorSuggestionItem, CodeEditorSuggestionItemKind, CollapsableSection, Collapse, ColorPicker, ColorPickerInput, type Column, Combobox, type ComboboxOption, type CompletionItem, type CompletionItemGroup, CompletionItemKind, type ComponentSize, ConfirmButton, ConfirmModal, type ConfirmModalProps, Container, ContextMenu, type ContextMenuProps, type ControlComponent, ControlledCollapse, Counter, type CustomCellRendererProps, type CustomComponentProps, type CustomControlProps, CustomHeadersSettings, CustomScrollbar, DEFAULT_ANNOTATION_COLOR, dom_d as DOMUtil, DataLinkButton, DataLinkInput, DataLinksContextMenu, type DataLinksContextMenuApi, type DataLinksContextMenuProps, DataLinksInlineEditor, DataLinksInlineEditorBase, type DataLinksInlineEditorBaseProps, DataSourceHttpSettings, DatePicker, type DatePickerProps, DatePickerWithInput, type DatePickerWithInputProps, DateTimePicker, DeleteButton, Divider, type DragHandlePosition, Drawer, Dropdown, type DropzoneFile, ElementSelectionContext, type ElementSelectionContextItem, type ElementSelectionContextState, EmptySearchResult, EmptyState, ErrorBoundary, ErrorBoundaryAlert, type ErrorBoundaryAlertProps, ErrorWithStack, EventBusPlugin, EventsCanvas, EventsWithValidation, FIXED_UNIT, FadeTransition, FeatureBadge, FeatureInfoBox, type FetchDataArgs, type FetchDataFunc, Field, FieldArray, type FieldArrayApi, FieldLinkList, type FieldProps, FieldSet, FieldValidationMessage, FileDropzone, FileDropzoneDefaultChildren, type FileDropzoneProps, FileListItem, type FileListItemProps, FileUpload, FilterInput, FilterPill, Form, type FormAPI, type FormInputSize, type FormatOptionLabelMeta, FormattedValueDisplay, FullWidthButtonContainer, Gauge, GlobalStyles, Graph, GraphContextMenu, GraphContextMenuHeader, GraphNG, type GraphNGLegendEvent, type GraphNGProps, GraphSeriesToggler, type GraphSeriesTogglerAPI, GraphWithLegend, Grid, type HighlightPart, HorizontalGroup, Icon, IconButton, type IconButtonVariant, type IconSize, type IconType, IndentationPlugin, InfoBox, InlineField, InlineFieldRow, InlineFormLabel, InlineLabel, InlineSegmentGroup, InlineSwitch, InlineToast, Input, type InputActionMeta, InputControl, InteractiveTable, JSONFormatter, JsonExplorer, KeyboardPlugin, Label$1 as Label, LegacyForms, LegacyInputStatus, Legend, Link, LinkButton, List, type LoadOptionsCallback, LoadingBar, type LoadingBarProps, LoadingPlaceholder, type LoadingPlaceholderProps, Marker, Menu, MenuGroup, type MenuGroupProps, MenuItem, type MenuItemProps, type MenuItemsGroup, type MenuProps, Modal, ModalHeader, type Props$R as ModalProps, ModalRoot, ModalTabContent, ModalTabsHeader, ModalsContext, ModalsController, ModalsProvider, type Monaco, type MonacoEditor, MultiCombobox, MultiSelect, type MultiSelectCommonProps, NO_DATA_COLOR, NewlinePlugin, NodeGraphDataFrameFieldNames, OK_COLOR, type OnSelectRangeCallback, type OnTagClick, PENDING_COLOR, PageToolbar, Pagination, PanelChrome, LoadingIndicator as PanelChromeLoadingIndicator, type LoadingIndicatorProps as PanelChromeLoadingIndicatorProps, type PanelChromeProps, type PanelChromeType, PanelContainer, type PanelContext, PanelContextProvider, PanelContextRoot, type PanelPadding, PlotLegend, type PlotSelection, type PlotTooltipInterpolator, PluginSignatureBadge, type PluginSignatureBadgeProps, Popover, type PopoverContent, PopoverController, Portal, PortalContainer, QueryField, type QueryFieldProps, REGION_FILL_ALPHA, RadioButtonDot, RadioButtonGroup, RadioButtonList, type RangeSelection1D, type RangeSelection2D, RangeSlider, ReactMonacoEditorLazy as ReactMonacoEditor, type ReactSelectProps, reactUtils_d as ReactUtils, RefreshPicker, RelativeTimeRangePicker, RenderUserContentAsHTML, RunnerPlugin, SCHEMA, ScrollContainer, type ScrollbarPosition, type SearchFunction, SearchFunctionType, SecretInput, SecretTextArea, SecureSocksProxySettings, Segment, SegmentAsync, SegmentInput, SegmentSection, SegmentSelect, Select, type SelectAsyncProps, type SelectBaseProps, type SelectCommonProps, SelectContainer, type SelectContainerProps, SelectMenuOptions, type SelectOptions, type SelectValue, type SelectableOptGroup, SelectionShortcutsPlugin, SeriesColorPicker, SeriesColorPickerPopover, SeriesColorPickerPopoverWithTheme, SeriesIcon, SeriesTable, type SeriesTableProps, SeriesTableRow, type SeriesTableRowProps, SeriesVisibilityChangeBehavior, SeriesVisibilityChangeMode, SetInterval, type SingleStatBaseOptions, SlatePrism, SlideOutTransition, Slider, Space, Sparkline, Spinner, Stack, StatsPicker, SuggestionsPlugin, type SuggestionsState, Switch, TLSAuthSettings, Tab, type TabConfig, TabContent, type TabProps, TabbedContainer, Table, TableCellInspector, TableCellInspectorMode, type TableCustomCellOptions, type TableFieldOptions, type TableFooterCalc, TableInputCSV, type TableSortByFieldState, TabsBar, Tag, TagList, TagsInput, Text, TextArea, TextLink, type Themeable, type Themeable2, TimeOfDayPicker, TimePickerTooltip, TimeRangeInput, TimeRangeLabel, TimeRangePicker, type TimeRangePickerProps, TimeRangeProvider, TimeSeries, TimeZonePicker, ToggleAllState, Toggletip, type Token, ToolbarButton, ToolbarButtonRow, Tooltip, TooltipPlugin, TooltipPlugin2, type TypeaheadInput, type TypeaheadOutput, UPLOT_AXIS_FONT_SIZE, UPlotChart, UPlotConfigBuilder, type UPlotConfigPrepFn, UnitPicker, UserIcon, type UserIconProps, type UserView, UsersIndicator, type UsersIndicatorProps, type ValidationEvents, type ValidationRule, ValuePicker, VerticalGroup, VerticalTab, VirtualizedSelect, type VirtualizedSelectAsyncProps, type VirtualizedSelectProps, VizLayout, type VizLayoutComponentType, type VizLayoutLegendProps, type VizLayoutProps, VizLegend, type VizLegendItem, VizLegendListItem, VizRepeater, type VizRepeaterRenderValueProps, VizTooltip, VizTooltipContainer, type WeekStart, WeekStartPicker, WithContextMenu, XYCanvas, ZoomPlugin, attachDebugger, buildScaleKey, calculateFontSize, clearButtonStyles, colors, index_d as commonOptionsBuilder, convertOldAngularValueMapping, createLogger, defaultIntervals, fieldMatchersUI, fuzzyMatch, getAvailableIcons, getCanvasContext, getCardStyles, getCellLinks, getDragStyles, getFieldTypeIcon, getFieldTypeIconName, getInputStyles, getPortalContainer, getScrollbarWidth, getSelectStyles, getTagColor, getTagColorIndexFromName, getTagColorsFromName, getTextColorForAlphaBackground, getTextColorForBackground, getTheme, getWeekStart, graphFieldOptions, graphTickFormatter, graphTimeFormat, hasValidationEvent, isIconSize, isWeekStart, linkModelToContextMenuItems, logOptions, makeFragment, makeValue, measureText, mockTheme, mockThemeContext, preparePlotFrame, regexValidation, resetSelectStyles, sharedInputStyle, sharedSingleStatMigrationHandler, sharedSingleStatPanelChangedHandler, sortedColors, mixins_d as styleMixins, stylesFactory, useElementSelection, useForceUpdate, useGraphNGContext, usePanelContext, useSplitter, useStyles, useStyles2, useTheme, useTheme2, validate, variableSuggestionToCodeEditorSuggestion, withErrorBoundary, withTheme, withTheme2 };
